...
if($image['name'] != ''){
	if(in_array($extension, $allowed_extension)){
		if($image['size'] < 5000000){
			...
			$select_image = "SELECT * FROM feature_image WHERE id=$id";
			...
			$update = "UPDATE feature_image SET name='$name' WHERE id=$id ";
			$update_result = mysqli_query($db_connect, $update);

			$image_name = "feature_image".$id.'.'.$extension;
			$new_location = 'uploaded/'.$image_name;
			move_uploaded_file($image['tmp_name'], $new_location);

			$update_image = "UPDATE feature_image SET image='$image_name' WHERE id=$id ";
			$update_image_result = mysqli_query($db_connect, $update_image);

			$_SESSION['success'] = "Image been changed successfully!";
			header('location: edit_feature_image.php?id='.$id);
